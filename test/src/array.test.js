...
describe('util.array 【数组操作】测试', function () {
  describe('util.array.copy 【数组拷贝】测试', function () {
    describe('单多插入：指定插入的元素类型', function () {
      it('数组插入：copy(["a", "b", "c"], [0, 1, 2]) 的序列化结果等于"0,1,2,a,b,c"', function () {
        util.array.copy(["a", "b", "c"], [0, 1, 2]).toString(';').should.equal('0,1,2,a,b,c');
      });
      it('批量插入：copy(["a", "b", "c"], [0, 1, 2], {multi: true}) 的序列化结果等于"0,1,2,a,b,c"', function () {
        util.array.copy(["a", "b", "c"], [0, 1, 2], {multi: true}).toString().should.equal('0,1,2,a,b,c');
      });
      it('多数组插入：copy(["a", "b", "c"], [[0, 1, 2], [3, 4, 5]], {multi: true}) 的序列化结果等于"0,1,2,a,b,c"', function () {
        util.array.copy(["a", "b", "c"], [[0, 1, 2], [3, 4, 5]], {multi: true}).toString().should.equal('0,1,2,a,b,c');
      });
    });
